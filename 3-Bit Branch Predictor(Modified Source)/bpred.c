...
///////////////////////////////////////////////////////      pred is bpred_t
  ...
  case BPredComb:
    /* bimodal component */
    pred->dirpred.bimod = 
      bpred_dir_create(BPred3bit, bimod_size, 0, 0, 0);                        //////////////////////////***********************************[It is fixed]
    ...
    pred->dirpred.meta = 
      bpred_dir_create(BPred3bit, meta_size, 0, 0, 0);                        //////////////////////////***********************************[It is fixed]
...
bpred_dir_create (
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  struct bpred_dir_t *pred_dir;                                       ////////////////////////               pred_dir is bpred_dir_t
  unsigned int cnt;
  int flipflop;
  ...
  case BPred3bit:                                     //////***************************************************************************************[ it is fixed]
    ...
    if (!(pred_dir->config.bimod.table =
      ...
      fatal("cannot allocate 3bit storage");                                     //////***************************************************************************************[ it is fixed]
    /* initialize counters to weakly this-or-that */
    flipflop = 1;                                                                          //////***************************************************************************************IMPORTANT
...
#define BIMOD_HASH(PRED, ADDR)						\
  ((((ADDR) >> 19) ^ ((ADDR) >> MD_BR_SHIFT)) & ((PRED)->config.bimod.size-1))
    /* was: ((baddr >> 16) ^ baddr) & (pred->dirpred.bimod.size-1) */                          ////////////////********************************IMPORTANT

/* predicts a branch direction */
char *						/* pointer to counter */                                    /////////////////**********************NOTICE
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  ...
	  dir_update_ptr->dir.meta  = (*meta >= 4);                       //////////////////********************[It is fixed]
	  dir_update_ptr->dir.bimod = (*bimod >= 4);                     //////////////////////**********************VERY VERY IMPORTANT
	  dir_update_ptr->dir.twolev  = (*twolev >= 4);                       //////////////////********************[It is fixed]
	  if (*meta >= 4)                                    ///////////////////*********************?????????????????????????????????????????[It is fixed]
	    {
    ...
    case BPred3bit:                       ///////////////////////////////////////////////////////////////////*********************************************[It is fixed]
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  dir_update_ptr->pdir1 =                                ///////////////////////////////////****************************NOTICE that pdir1 accept the counter value
  ...
  if (pbtb == NULL)
    {
      ...
      return ((*(dir_update_ptr->pdir1) >= 4)                 ////////////////************************VERY IMPORTANT[It is fixed]************************************
	      ? /* taken */ 1
	      : /* not taken */ 0);
    }
  else
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  /* update state (but not for jumps) */                            ///////////////////********************************VERY VERY VERY IMPORTANT
  if (dir_update_ptr->pdir1)
    {
      if (taken)
	{
	  if (*dir_update_ptr->pdir1 < 7)                           ///////////////////********************************VERY VERY VERY IMPORTANT[It is fixed]
	    ++*dir_update_ptr->pdir1;
	}
      else
	{ /* not taken */
	  if (*dir_update_ptr->pdir1 > 0)                           ///////////////////********************************VERY VERY VERY IMPORTANT
	    --*dir_update_ptr->pdir1;
	}
    }
  ...
  if (dir_update_ptr->pdir2)
    {
      if (taken)
	{
	  if (*dir_update_ptr->pdir2 < 7)                           ///////////////////********************************VERY VERY VERY IMPORTANT[It is fixed]
	  ...
	  if (dir_update_ptr->dir.twolev == (unsigned int)taken)
	    {
	      ...
	      if (*dir_update_ptr->pmeta < 7)                           ///////////////////********************************VERY VERY VERY IMPORTANT[It is fixed]
